...
impl CompUI {
    pub fn new(
        ...
        root_children.insert_at_top(game_board_visual);
        let selection_visual = game_board.selection_visual();
        root_children.insert_at_top(selection_visual);
    ...
    pub fn hit_test(&self, point: &Vector2) -> Option<TileCoordinate> {
        ...
        self.game_board.hit_test(&point)
    }
    ...
    pub fn reset(&mut self, grid_size_in_tiles: &GridSize) -> Result<()> {
        ...
        for visual in self.game_board.tiles() {
            visual.set_brush(
                self.assets
    ...
    pub fn update_tile_with_mine_count(
        ...
        if num_mines > 0 {
            ...
            shape_visual.set_relative_size_adjustment(Vector2::one());
    ...
    pub fn play_mine_animations(
        ...
        let parent_children = self.game_board.root().children();
        while !mine_indices.is_empty() {
            let mine_index = *mine_indices.front().unwrap();
            self.play_mine_animation(&parent_children, mine_index, current_delay)?;
    ...
    fn compute_scale_factor_from_size(&self, window_size: &Vector2) -> f32 {
    ...
    fn play_mine_animation(
        &self,
        parent_children: &VisualCollection,
        index: usize,
        delay: Duration,
    ) -> Result<()> {
        let visual = self
        ...
        animation.insert_key_frame(0.0, Vector3::one());
        animation.insert_key_frame(0.7, Vector3::new(2.0, 2.0, 1.0));
        animation.insert_key_frame(1.0, Vector3::one());
