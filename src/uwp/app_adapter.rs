...
impl App {
    pub fn new(app_inner: Box<dyn UwpApp>) -> winrt::Result<Self> {
        unsafe {
            let raw = AppRc::new(app_inner);
...
struct AppRc {
    ...
    inner: Box<dyn UwpApp>,
}

impl AppRc {
    pub fn new(inner: Box<dyn UwpApp>) -> *mut Self {
    ...
    pub fn get_inner(&mut self) -> &mut dyn UwpApp {
        self.inner.as_mut()
    }
...
impl impl_App_IFrameworkView {
    ...
    unsafe extern "system" fn set_window(
        ...
        let result = (*this).base.as_mut().unwrap().get_inner()
            .set_window(&window)
            .map_or_else(|e| e.code(), |()| winrt::ErrorCode(0));
    ...
    unsafe extern "system" fn run(
        ...
        let this: *mut Self = this.as_raw() as _;
        (*this).base.as_mut().unwrap().get_inner()
            .run()
            .map_or_else(|e| e.code(), |()| winrt::ErrorCode(0))
    }
